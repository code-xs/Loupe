...
> **报告版本**：**v1.1**（基于 v1.0 + `QUALITY-AUDIT-REVIEW.md` 的反驳与补充交叉核验）
> **审计日期**：2026-04-20
...
| 状态机可靠性 | **4.5**（v1.0 为 5） | Phase 早退语义未结构化、字段污染（P4→fanout_mode）、step-pause 无写回协议 → 系统性失锚 |
...
**综合得分：5.8/10**（v1.0 6.0/10）— 在 Review 5.1/5.3/5.4 三处系统性风险加入后，状态机可靠性下调，整体可执行可靠性进一步暴露。
...
| `core/`（workflow.xml + 4 个 yaml + core-rules） | **2** | **5** | **7** | 3（v1.0 4，撤回 m7） |
| `agents/`（8 个） | 0 | 3 | 5 | 4 |
| `phases/`（P1–P6） | 2 | **6**（v1.0 5，新增 C10） | 7 | 5 |
| `templates/`（14 个） | 0 | 1 | 6 | 6 |
| `reference/`（4 个） | 0 | 0 | 3 | 2 |
| `functionality-deep-dive/`（25+） | 1 | 4 | 7 | 4 |

合计：**5 Blocker + 21 Critical + 39 Major + 27 Minor = 92 项**（v1.0 共 41 项，v1.1 因新增字段化复盘 + 全链路抽象升级显著扩张）。
...
#### B1\*. 【v1.1 升级】Phase 早退（abort）语义未结构化协议 — 全链路系统性根因
- **位置**：`core/workflow.xml` step 4 ↔ `phases/p3-root-cause.md` ×4 处 + `phases/p6-verification.md` + `phases/p2-spec-definition.md` + `functionality-deep-dive/phases/f5-defensive-fix-design.md`
- **证据**：
...
而 P3/P6 等多处仅以自然语言"阶段结束，返回编排器"早退（grep 实测 P3 4 处、P6 1 处、Deep-Dive F5 1 处），**全程没有显式 `current_phase_result = ABORT` 的赋值**。
...
- **Fix Sketch**：
  ...
  2. 在受影响的 P2/P3/P6/Deep-Dive 各早退动作前**统一插入** `<action>设置 current_phase_result = ABORT</action>`。
...
#### C3. `challenger` 包装层 dimension_set 枚举与基座不一致
...
- **Contract Surface**：subagent-io × tag-dsl
- **Failure Mode**：Deep-Dive 注入 `deep-dive-7d` 时主 wrapper 校验为非法。
- **Fix Sketch**：把 `deep-dive-7d` 加入主 wrapper（推荐：让主 wrapper 也透传 DEEP_DIVE 场景）。
...
- **Failure Mode**：`fanout_mode` 在 P3 是 RCA fan-out 三档枚举（`simple-single / medium-challenge / complex-arbitrated`），P4 写为 Fix 阶段值（`single-design / parallel-3 / contested-arbitrated`） → P6 失败回流到 P3 强制 `complex-arbitrated` 判断时，读取被污染的值 → 路由错乱。
- **Repro**：P3 完成（fanout_mode = simple-single）→ P4 进入并写 fanout_mode = single-design → P5 → P6 失败 root_cause_not_closed → reroute 回 P3 → P3 step 1 检查 fanout_mode 当前为 single-design → 升级判断条件不匹配 → 升级失败。
...
- **Compatibility**：旧会话需补一次性脚本：若 `current_state ∈ {Fix-Designing, Fix-Implementing, Verifying}` 且 `fanout_mode ∈ Fix 枚举`，则迁移到新字段并把 `rca_fanout_mode` 还原为 P3 完成时值。
...
### 立刻修复（本周）— Blocker / Critical（含 v1.1 新增 C10/C11）

1. **【根因 A】Phase 早退 = ABORT 强协议**：core-rules.xml 增加约束；P2/P3/P6/Deep-Dive F5 全部早退动作前插入 `set current_phase_result = ABORT`。
...
8. **【C10】P4 字段隔离**：新增 `fix_fanout_mode`（或重命名 `fanout_mode → rca_fanout_mode`）。
9. **【C11】step-pause result_field 协议**：core-rules.xml 增加该参数；workflow-status-template 列出所有用户输入字段。

### 本月内（Major）— 契约和模板治理

10. 统一 arbiter / challenger wrapper 与共享基座的 scene/dimension_set 枚举（含 DEEP_DIVE / deep-dive-7d）。
...
紧急度：高    │ B1* ABORT 协议（系统性根因）                      │ M3 占位变量未赋值
              ...
              │                                                    │
紧急度：中    │ C3 dimension_set  C4 Merged 枚举                   │ M9 子工作流隔离
              │ C5 状态枚举漂移   C6 计数器缺失                    │ M14 平台适配过度承诺
              │ C8 文档漂移                                        │
              │ ★ M16 config_source 键漂移                         │
              │                                                    │
紧急度：低    │ M1/M2 命名/字段   M4 wrapper 场景                  │ m1-m14 其他 minor
              │ M15 reference 双源                                 │
```

★ = v1.1 新增（基于 Review 反馈核验）

---

## 九、关键洞察（v1.1 强化）

1. **设计意图 vs 实施落差最大的环节是"Phase 早退/回流的协议化"**。原 v1.0 把它窄化成 P3 reroute 与 stepsCompleted 互锁，Review 抽象为系统性 ABORT 协议缺失，覆盖 P2/P3/P6/Deep-Dive 全链路。
2. **跨阶段字段命名复用是隐形雷区**（fanout_mode 即是典型）。LLM 编排不会像类型系统一样阻止复写，必须在 schema 层做命名空间隔离。
3. **共享基座 + 包装层**没有任何静态校验保证 wrapper 与 base 的 scene/dimension/枚举对齐——典型 LLM Agent 工程"跨文件契约失守"。
...
| # | 动作 | 工作量 | 立即收益 |
|---|---|---|---|
| 1 | 【B1\*】ABORT 协议落地（core-rules.xml 强约束 + 6 处 phase 早退点改写） | 0.7d | 解决全链路 stepsCompleted 失锚 |
...
**报告完成（v1.1）** · 致谢 [QUALITY-AUDIT-REVIEW.md](./QUALITY-AUDIT-REVIEW.md) 的专业核验。如需直接落地这些修复，可按"推荐落地动作清单"逐项创建 PR；建议优先级为 **B1\* + C10 + C11 三件套**（系统性根因 + 字段污染 + IPC 协议），约 1.7 人日即可解决最深层的协议缺失。
