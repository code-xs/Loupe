...
| 5 | 🟢 P2 | 协议契约真实性 | 真实 `workflow-status-template.yaml` 已有 L16 `non_bug_reflow_count: 0`，与 PR-1 新增的 `non_bug_user_choice`（L353） / `non_bug_context`（L338）同属 Non-Bug 流程相关字段，但 PR-1 修订理由没有说明三者的职责正交关系；reviewer / 后续 PR 实施者可能误用或误删。 | PR-1 §2.2.2 修订理由段追加一行三字段对照：**"职责正交：`non_bug_reflow_count` = 跨轮回流次数（已有）；`non_bug_user_choice` = 当前轮 step-pause 用户选择（v4.1 新增，白名单镜像）；`non_bug_context` = 当前轮 Non-Bug 判定上下文文本（v4.1 新增，**顶层 schema 字段**，由 PR-3 写入、供编排器 case Non-Bug 的 step-pause 标题占位读取）。"** | [pr1-schema-protocol.md:L358-L363](./pr1-schema-protocol.md#L358-L363) / [workflow-status-template.yaml:L16](../../core/workflow-status-template.yaml#L16) |
| 6 | 🟢 P2 | 协议契约真实性 | PR-1 §3 DoD 子集第 5 项写"`allowed_keys` 与 `core/default-config.yaml` 中已声明键名**互为子集**"，字面等价于"两个集合相等"，过严。实际 `config-schema.yaml` 应是 superset（允许定义"将来要支持但尚未默认配置"的键，如 deep-dive `output_*`）；`default-config.yaml` 应是 subset（实际有默认值的键）。当前措辞会导致：未来在 `config-schema.yaml` 增加键名（无 default）时，DoD 校验 fail。 | 把"互为子集"改为"`default-config.yaml` 中所有键名 ⊆ `config-schema.yaml.allowed_keys`（schema 是 superset，default 是 subset）"。该问题只存在于 `pr1-schema-protocol.md` 子集投影写法中，主文档 §5.1 第 5 项措辞正确，**无需同步修订主文档**。 | [pr1-schema-protocol.md:L531](./pr1-schema-protocol.md#L531) / [QUALITY-AUDIT-CONSTRUCTION-PLAN-v2.2.md:L515](../../QUALITY-AUDIT-CONSTRUCTION-PLAN-v2.2.md#L515) |
...
**影响**：编辑器按 fenced reference 跳转指向错误行；reviewer grep `</execution>` 会怀疑 PR-1 误删该标签；§5.1 第 15 项"持久化字段表纯净性"如果按行号锚点抽样验证会 fail。
...
> - `non_bug_context`（v4.1 新增）= 当前轮 P2 Non-Bug 判定上下文文本（顶层 schema 字段，由 PR-3 写入、供编排器 case Non-Bug 的 step-pause 标题占位读取）
...
**修订建议**：改为"`default-config.yaml` 中所有键名 ⊆ `config-schema.yaml.allowed_keys`（schema 是 superset，default 是 subset）"。该问题只存在于 `pr1-schema-protocol.md` 的 PR-1 子集投影写法中，不应外推为主文档 §5.1 第 5 项的 shared bug。
...
## 本地变更代码 Review（v1.3 增量）

> **审阅范围**：当前工作区已落盘的 PR-1 本地代码变更
> `core/core-rules.xml` / `core/workflow-status-template.yaml` / `core/default-config.yaml` / `core/config-schema.yaml` / `scripts/migrate-workflow-status-v3-to-v4.py`
>
> **审阅目标**：核对本地实现是否与 `pr1-schema-protocol.md` 的协议意图闭合，是否可作为独立 patchset 合入。

### 增量结论

- 当前本地实现已完成 PR-1 的大部分协议层落盘：XML / YAML 语法可解析，`default-config.yaml` 与 `config-schema.yaml.allowed_keys` 当前集合一致，迁移脚本文件也已实际落盘并具备可执行权限。
- 但若按"当前本地变更可独立提交"口径审查，仍存在 **2 个 P0** 与 **2 个 P1**：
  - **P0**：`<step-pause>` 新契约已把现有 `core/workflow.xml` 中多个 step-pause 调用判为非法；
  - **P0**：`current_state` 的"单一权威源"枚举与主编排器实际写入/分支消费不一致；
  - **P1**：迁移脚本依赖 `ruamel.yaml`，但仓库内尚无正式依赖闭环；
  - **P1**：迁移脚本在 `specialized_workflow` 锚点缺失时会静默尾插，违反 preserve 区域约束。
- 结论：**当前代码不建议直接作为 PR-1 独立合入**；需先收口上述 4 项，再宣称 PR-1 协议层已自洽。

### 增量 Findings

| No. | Pri | 维度 | Issue Title | Suggestion | Code Link |
|---|---|---|---|---|---|
| 12 | 🔴 P0 | 协议契约闭合性 | `core-rules.xml` 已将 `<step-pause>` 的 `result_field` / `allowed_values` 定义为必填，但 `core/workflow.xml` 现有多个 `<step-pause>` 仍只有 `title` / `option`。协议层刚收紧，主编排器立即不满足自身 DSL。 | 若 PR-1 允许独立合入，则需在同一 patchset 中同步补齐 `core/workflow.xml`（以及同类 workflow）所有 step-pause 的 `result_field` 与 `allowed_values`；否则不要在 PR-1 先把二者标为 `required="true"`。 | [core-rules.xml:L126-L145](../../core/core-rules.xml#L126-L145) / [workflow.xml:L103-L116](../../core/workflow.xml#L103-L116) / [workflow.xml:L142-L156](../../core/workflow.xml#L142-L156) |
| 13 | 🔴 P0 | Schema 自洽性 | `workflow-status-template.yaml` 头部将 `current_state` 权威枚举写为 `... / RCA-Designing / ... / Done`，但主编排器仍在写入并消费 `Closed` 与 `RCA-InProgress`。新增的"单一权威源"与运行时状态机冲突。 | 先统一状态名，再宣称"单一权威源"：要么同步修改 `core/workflow.xml` / phase 写入侧为新枚举，要么把模板中的枚举先对齐当前真实运行时值。 | [workflow-status-template.yaml:L4-L10](../../core/workflow-status-template.yaml#L4-L10) / [workflow.xml:L122-L123](../../core/workflow.xml#L122-L123) / [workflow.xml:L158-L174](../../core/workflow.xml#L158-L174) |
| 14 | 🟡 P1 | 可执行性 | 迁移脚本硬依赖 `ruamel.yaml`，但仓库内未发现 `requirements*.txt` / `pyproject.toml` 等正式依赖声明；`install.sh` 也只做软链安装，不会准备 Python 依赖。实测在当前环境会直接报 `requires ruamel.yaml`。 | 将 `ruamel.yaml` 纳入正式依赖管理或安装脚本；至少补 README / 实施文档前置依赖说明，否则"迁移脚本可执行" DoD 不能成立。 | [migrate-workflow-status-v3-to-v4.py:L33-L38](../../scripts/migrate-workflow-status-v3-to-v4.py#L33-L38) / [install.sh:L16-L22](../../install.sh#L16-L22) / [install.sh:L63-L73](../../install.sh#L63-L73) |
| 15 | 🟡 P1 | 迁移安全性 | 迁移脚本要求新字段必须插入到 `specialized_workflow` 之前以保持在 `[PRESERVE_FORMAT]` 区间内，但 `_find_insert_pos()` 在锚点缺失时返回 `len(doc)`，后续 `insert()` 会静默退化为尾插。 | `strict` 模式下应在缺失 `specialized_workflow` 锚点时直接报错退出，而不是静默 fallback 到文件尾。 | [migrate-workflow-status-v3-to-v4.py:L50-L53](../../scripts/migrate-workflow-status-v3-to-v4.py#L50-L53) / [migrate-workflow-status-v3-to-v4.py:L68-L105](../../scripts/migrate-workflow-status-v3-to-v4.py#L68-L105) |

### 增量验证记录

- `core/core-rules.xml`：XML 解析通过。
- `core/default-config.yaml` / `core/workflow-status-template.yaml` / `core/config-schema.yaml`：YAML 解析通过。
- `core/default-config.yaml` 与 `core/config-schema.yaml.allowed_keys`：当前键集合一致，无缺口。
- `scripts/migrate-workflow-status-v3-to-v4.py`：`py_compile` 通过，但在当前环境运行会因缺少 `ruamel.yaml` 失败。
...
| v1.2 | 2026-04-20 | 修正 3 处事实错误：① Finding #1 中 `§5.1` 条目编号 `14 → 15`；② Finding #5 中 `non_bug_context` 的字段层级说明改为"顶层 schema 字段"；③ Finding #6 删除对主文档 `§5.1` 第 5 项 shared bug 的误判，收敛为仅针对 `pr1-schema-protocol.md` 子集投影措辞的问题 |
| v1.3 | 2026-04-20 | 新增"本地变更代码 Review（v1.3 增量）"：基于当前工作区已落盘代码重新审查 PR-1 实现，确认 4 项有效问题，其中 P0 2 项（`step-pause` 必填参数与主编排器未同步、`current_state` 权威枚举与运行时状态机冲突）/ P1 2 项（迁移脚本 `ruamel.yaml` 依赖未闭环、缺失 `specialized_workflow` 锚点时静默尾插）；并补充 XML/YAML 解析、schema 键集合一致性与迁移脚本可执行性验证记录 |
