# Mobile B2C 质量工作流 — 深度质检报告 v1.2.1

> **审计范围**：`/Users/bytedance/Code/loupe/mobile-qa-workflow/` 共 **73 个工作流源码文件**（不含 `QUALITY-AUDIT-*.md` 审计衍生文档；目录全量文件数随审计文档增加而变化，故以源码口径为准）
...
> **报告版本**：**v1.2.1**（基于 v1.2 + `QUALITY-AUDIT-REPORT-v1.2-REVIEW.md` 第三轮微修订）
> **审计日期**：2026-04-20
...
| # | v1.2 中的问题 | v1.2.1 处理 |
|---|---|---|
| 1 | "全部 74 个文件"未声明排除规则；且实测目录现状为 79 个全量 / 73 个工作流源码 | **改为"73 个工作流源码文件，不含 `QUALITY-AUDIT-*.md` 衍生文档"**；附录 A 同步收紧 |
| 2 | C10 Compatibility 引用了仓库当前不存在的 `phase_history` 字段 | **明确依赖关系**：`phase_history` 是 B1\* 提议引入的新字段，C10 迁移依赖 B1\* 先落地；并新增"备选项：在 P3 step 7 显式快照 `rca_fanout_mode_snapshot`" |
| 3 | 落地动作清单 #1 把"5 处"与"= 共 6 处"写在同一行，计数自相矛盾 | **拆分两口径**：显式 `阶段结束，返回编排器` 5 处（P3 ×4 + P6 ×1） + P2 隐式 step-pause 1 处 = 共需补 6 处 ABORT 标记 |

> **未变更内容**：v1.2 的 4 项重大修订（统计双口径、C10 枚举名更正、B1\* 证据链收窄、C3→M17 降级）全部保留；评分、唯一问题数 43、工作量 5.2d 均不变。
...
> **统计口径说明（v1.2 新增 / v1.2.1 强化）**：
> 1. 本表为"**模块影响计数**"——一个缺陷若同时触及 N 个模块，则在 N 个模块各计 1 次（允许重复）。"**唯一问题数**"另列，避免读者误读。
> 2. **审计对象口径**：以下 7 个模块共 **73 个工作流源码文件**，不含同目录下的 `QUALITY-AUDIT-*.md` 审计衍生文档。
...
- **【v1.2.1 修订】Compatibility**（写明依赖关系）：旧会话需补一次性脚本：若 `current_state ∈ {Fix-Designing, Fix-Implementing, Verifying}` 且 `fanout_mode ∈ {single-proposer, challenged-proposer, contested-arbitrated}` 则视为 Fix 枚举 → 迁移到 `fix_fanout_mode`。但**还原 `rca_fanout_mode` 旧值受限于当前仓库无历史快照字段**：
  - 当前 `workflow-status-template.yaml` 仅有 `stepsCompleted: []` 与 `lastStep: null`，**无 `phase_history`，也无 `rca_fanout_mode_snapshot`**；`lastStep` 只能告知"最近完成的阶段"，无法可靠还原 P3 完成时的 RCA fan-out 三档值。
  - **方案 A（推荐）**：迁移 C10 之前先落地 B1\* 引入的 `phase_history`（允许重复的阶段历史），从中检索最近一次 P3 完成时的 fanout_mode 值。
  - **方案 B（备选）**：在 P3 step 7 显式增加 `<action>更新 {workflow_status}：rca_fanout_mode_snapshot = {fanout_mode}</action>`，作为独立快照字段；C10 迁移脚本读取该字段还原。
  - **若以上均未落地**：旧会话只能"部分推断"（按 `last_known_p3_state` 默认值兜底），不能承诺 100% 自动还原；建议对受影响的存量会话走人工核验。
...
| # | 动作 | 工作量 | 立即收益 |
|---|---|---|---|
| 1 | 【B1\*】ABORT 协议落地（core-rules.xml 强约束 + 主链路改写共 6 处补 ABORT 标记：**显式 `阶段结束，返回编排器` 5 处 = P3 ×4 + P6 ×1**；**P2 隐式 step-pause 1 处**） | 0.7d | 解决主链路 stepsCompleted 失锚 |
...
### 覆盖文件清单（73 个工作流源码文件，**不含 `QUALITY-AUDIT-*.md` 审计衍生文档**）
...
> **统计口径**：
> - 全目录文件总数随审计文档增加而变化（v1.2.1 撰写时为 79）；
> - **审计实际针对的源码文件数 = 73**（v1.2 曾写 74，经 v1.2.1 复核更正）。
...
| 类型 | 条目 | 变更说明 |
|---|---|---|
| 范围口径修正 | 标题 + 第二章脚注 + 附录 A 覆盖清单 | "全部 74 个文件" → "73 个工作流源码文件，不含 `QUALITY-AUDIT-*.md` 衍生文档"；明确目录全量数（79）与源码数（73）的双口径 |
| 兼容性收紧 | C10 Compatibility | 明确 `phase_history` 当前不存在，是 B1\* 提议引入的新字段；C10 迁移依赖 B1\* 先落地；新增方案 B（P3 step 7 显式 `rca_fanout_mode_snapshot`）与方案 C（人工核验兜底） |
| 计数口径自洽 | 落地动作清单 #1 | "5 处 = 共 6 处"自相矛盾 → 拆分为"显式 5 处（P3 ×4 + P6 ×1） + P2 隐式 step-pause 1 处 = 共 6 处" |
| 评分调整 | 无 | 综合分 5.9、跨工作流一致性 5.0、唯一问题数 43、工作量 5.2d 全部不变 |
| 缺陷台账 | 无新增/无降级 | 本轮纯报告工程学收口 |
