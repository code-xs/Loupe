...
- `workflow-status.yaml` 是动态路由唯一可信状态源；`analysis_complexity`、`fanout_mode`、`fix_strategy_mode`、`reroute_target_phase`、重试计数等字段由阶段文件写回，由主编排器读取并执行。
...
- 至少持久化：`workflow_version`、`analysis_complexity`、`fanout_mode`、`fix_strategy_mode`、`reroute_target_phase`、`rca_retry_count`、`fix_retry_count`
